...
export const createFermentable = async (breweryUuid, fermentableData) => {
  ...
  if (fermentableUuid) { // if user passed a UUID for the new fermentable
    return fermentableUuid;
  }
  const [newFermentable] = await Tables.fermentable.select({fermentableKey: insertId});
  localCache.invalidate("fermentable");
  return newFermentable.fermentableUuid; 
}

export const isExistingFermentableAttribute = localCache.isExistingTableAttribute("fermentable", getFermentables);